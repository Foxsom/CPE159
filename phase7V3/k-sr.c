...
void ExecSR(int code, int arg){
	int * codeAddress;
	int * stackAddress;
	int codeAddressP = NONE; // store i value for when NONE
	int stackAddressP =NONE; // stores i value when NONE
  int i;
	...
	for(i = 0; i<PAGE_NUM; i++){
		...
		if(page_user[i]==NONE){
			...
			if (codeAddressP==NONE){
				codeAddressP  = i;	
	
			}
			else if((int)stackAddressP==NONE){
			  stackAddressP = i;
     ...
     // codeAddress = (int *)((i* PAGE_SIZE)+RAM);
			page_user[codeAddressP] = run_pid;
      page_user[stackAddressP] = run_pid;
      break;
		}		
	}
...
void WrapperSR(int pid, int handler, int arg){
  ...
  int *tempTP;
  int point;
	//tempTP = (int *)pcb[pid_run].trapframe_p; 
  ...
  tempTP --;
	...
	*tempTP = handler;
	tempTP --;
	*tempTP = pcb[pid].trapframe_p->eip;
